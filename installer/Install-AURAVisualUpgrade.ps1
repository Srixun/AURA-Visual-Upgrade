...
function Download-ReleasePayload([object]$Release) {
    ...
    try {
        ...
        return [pscustomobject]@{ Path=$archive; Hash=$expected; Version=([string]$Release.tag_name).TrimStart('v'); TempRoot=$tempRoot; Source='GitHub Releases' }
    } catch {
...
function Get-BundledPayload {
    ...
    return [pscustomobject]@{ Path=$payloadPath; Hash=$expected; Version=$null; TempRoot=$null; Source='bundled package' }
}
...
function Show-Status([string]$TargetPath) {
    ...
    [pscustomobject]@{
        ...
        LatestVersion = if ($latest) { ([string]$latest.tag_name).TrimStart('v') } else { 'Unavailable/offline' }
        UpdateAvailable = if ($latest -and $version) { [version](([string]$latest.tag_name).TrimStart('v')) -gt [version]$version } else { $false }
    } | Format-List
}
...
function Install-WithUpdateCheck([string]$TargetPath, [bool]$RequireRemoteUpdate) {
    ...
    try {
        if ($SkipUpdateCheck) {
            ...
            if ($installedVersion -and [version]$latestVersion -le [version]$installedVersion) {
                Write-Host "AURA Visual Upgrade $installedVersion is already current." -ForegroundColor Green
                return
            }
